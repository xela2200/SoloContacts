...
namespace SoloContacts.Core.Data
{
    ...
    /// </remarks>
    //[Serializable()]
    public class SmartDate : IComparable
    {
        ...
        public SmartDate()
        {
            _emptyValue = GetEmptyValue(true);
            _format = null;
            _initialized = false;
        ...
        private static bool TryStringToDate(string value, EmptyValue emptyValue, ref DateTime result)
        {
            ...
            else
            {
                ...
                else if (value.Trim().Length == 8) //YYYYMMDD
                {
                    DateTime.TryParse
                        (
                        string.Format
                            (
                                "{0}/{1}/{2}",
                                value.Substring(4, 2), //MM
                                value.Substring(6, 2), //DD                         
                                value.Substring(0, 4)  //YY
                            )
                        , out tmp
                        );
